...
public class EmailUtils {
	...
	public static void sendEmail(String email, String title, String content) {
		...
		try {
			// 3. 创建一封邮件
			Message message = createMsg(session, myAccount, email, title,
					content);
	 ...
	 *            用户对象
	 */
	private static Message createMsg(Session session, String sendMail,
			String email, String title, String content) throws Exception {
		// 1. 创建一封邮件
		Message message = new MimeMessage(session);
		// 2. From: 发件人
		message.setFrom(new InternetAddress(sendMail, "千峰成都重要通知", "UTF-8"));
		// 3. To: 收件人（可以增加多个收件人、抄送、密送）
		message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(
				email, "致尊敬的用户", "UTF-8"));
		// 4. Subject: 邮件主题
		message.setSubject(title);// (title, "UTF-8");
